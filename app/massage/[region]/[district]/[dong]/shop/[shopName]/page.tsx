...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const charSum = (locationPrefix + "momentrest_shop_clean_100").split('').reduce((acc, char) => acc + char.charCodeAt(0), 0);
  const variantIndex = charSum % 50;

  // 타이틀: '출장'과 '마사지'가 절대 붙어있지 않고 분산된 50가지 조합 (샵 이름 완전 제외)
  const titleVariants = [
  ...
  return {
    ...
    alternates: {
      canonical: `https://momentrest.netlify.app/massage/${region}/${encodeURIComponent(district)}/${encodeURIComponent(dong)}/shop/${shopName}`,
    },
    openGraph: {
      title: pageTitle,
      description: pageDescription,
    ...
    <div className="bg-[#fff5f7] text-[#2f3542] min-h-screen flex flex-col font-sans selection:bg-pink-400 selection:text-white pb-28">
      ...
      <main className="max-w-4xl mx-auto px-4 py-8 w-full flex-1 space-y-8">
        ...
        <div className="text-center pt-4 pb-12">
          <Link href={`/massage/${region}/${encodeURIComponent(district)}/${encodeURIComponent(dong)}`} className="text-xs text-gray-500 hover:text-pink-600 font-semibold transition-colors">
            ← 이전 동 목록으로 돌아가기
          </Link>
        </div>
