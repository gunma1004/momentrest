...
export async function generateMetadata({ params, searchParams }: PageProps): Promise<Metadata> {
  try {
    ...
    const charSum = (locationPrefix + "momentrest_district_clean_100").split('').reduce((acc, char) => acc + char.charCodeAt(0), 0);
    const variantIndex = charSum % 50;

    // 타이틀: '출장'과 '마사지'가 절대 붙어있지 않고 중간 단어로 분산된 50가지 조합 (사이트명 제외)
    const titleVariants = [
      ...
      `${locationPrefix} 맞춤형 홈케어 방문 출장 스웨디시 마사지 안내`,
      `${locationPrefix} 1:1 방문 홈케어 출장 타이 마사지 제휴점 정보`,
      ...
      `${locationPrefix} 전문 방문 홈케어 출장 웰니스 마사지 가이드`,
      ...
      `${locationPrefix} 소프트 케어 방문 홈케어 출장 마사지 안내`,
      `${locationPrefix} 쾌적한 실내 방문 홈케어 출장 테라피 마사지`,
      `${locationPrefix} 명품 방문 홈케어 출장 스웨디시 프로그램 정보`
    ];

    // 디스크립션: '출장' 키워드를 완전히 제외하고, 마사지 중심의 정제된 50가지 조합
    const descriptionVariants = [
      `${locationPrefix} 지역에서 편안하게 이용할 수 있는 전문 마사지 제휴업체 정보와 쾌적한 프로그램 안내를 확인해 보세요.`,
      `${locationPrefix} 인근에서 차별화된 힐링 마사지 코스를 찾고 계신다면 신뢰도 높은 제휴 요금표를 만나보세요.`,
      `${locationPrefix} 맞춤형 바디케어 안내입니다. 일상의 피로를 말끔히 풀어주는 전문 매장의 릴렉스 마사지 프로그램을 비교해 보세요.`,
      `${locationPrefix} 전 지역에서 신속하게 이용 가능한 정찰제 마사지 및 프라이빗 제휴샵 정보를 빠르고 정확하게 안내해 드립니다.`,
      `${locationPrefix} 최고의 휴식을 선사하는 전문 테라피스트들의 맞춤형 마사지 코스. 다양한 제휴 혜택을 확인해 보세요.`,
      ...
      `${locationPrefix} 인근 마사지 추천 업소 안내. 엄선된 힐러들의 전문적인 바디 케어 프로그램과 이용 요금을 확인해 보세요.`,
      `${locationPrefix} 전용 맞춤 마사지 가이드. 편안한 공간에서 즐기는 힐링 테라피 정보를 지금 바로 비교해 보세요.`,
      ...
      `${locationPrefix} 마사지 전문 제휴 플랫폼입니다. 엄선된 힐링 프로그램과 상세한 가격 정보를 확인해 보세요.`,
      `${locationPrefix} 인근에서 가장 만족도 높은 마사지 숍 정보와 체계적인 바디 케어 코스 안내를 제공해 드립니다.`,
      `${locationPrefix} 웰니스 힐링 마사지 안내. 일상의 스트레스를 편안하게 해소해 주는 전문 테라피 서비스를 확인해 보세요.`,
      ...
      `${locationPrefix} 맞춤형 힐링 마사지 가이드. 투명한 정보와 실속 있는 제휴 혜택을 확인해 보세요.`,
      `${locationPrefix} 인근에서 검증된 마사지 제휴점 정보. 편안한 분위기 속에서 즐기는 프리미엄 바디 케어 안내입니다.`,
      `${locationPrefix} 전용 감성 마사지 코스 소개. 지친 하루의 끝을 포근하게 채워줄 힐링 테라피를 확인해 보세요.`,
      ...
      `${locationPrefix} 전용 마사지 프로그램 안내. 투명하고 믿을 수 있는 제휴 숍들의 상세 요금을 확인해 보세요.`,
      ...
      `${locationPrefix} 인근 마사지 숍 실시간 정보. 철저한 위생 관리와 편안한 힐링 코스를 만나보세요.`,
      ...
      `${locationPrefix} 인근에서 인기 있는 프리미엄 마사지 코스와 이용 요금을 확인해 보세요.`,
      ...
      `${locationPrefix} 엄선된 지역별 전문 마사지 제휴처 정보.`
    ];
    ...
    return {
      ...
      alternates: {
        canonical: `https://momentrest.netlify.app/massage/${region}/${encodeURIComponent(districtName)}${dongName ? `?dong=${encodeURIComponent(dongName)}` : ""}`,
      },
      openGraph: {
        title: finalTitle,
        description: finalDescription,
        url: `https://momentrest.netlify.app/massage/${region}/${encodeURIComponent(districtName)}${dongName ? `?dong=${encodeURIComponent(dongName)}` : ""}`,
...
export default async function RegionalDetailPage({ params, searchParams }: PageProps) {
  ...
  const jsonLd = {
    ...
    "@type": "LocalBusiness",
    "name": `${fullTitle} 힐링 테라피 & 홈케어 안내`,
    "description": `${fullTitle} 지역 프라이빗 테라피 및 힐링 바디케어 제휴업체 정보 제공`,
    "url": `https://momentrest.netlify.app/massage/${region}/${encodeURIComponent(districtName)}`,
    ...
    <div className="bg-[#fff5f7] text-[#2f3542] min-h-screen flex flex-col font-sans selection:bg-pink-400 selection:text-white pb-24">
      ...
      <main className="max-w-4xl mx-auto px-4 py-8 w-full flex-1 space-y-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {localShops.map((lShop) => (
              <div key={lShop.id} className="bg-white border border-pink-200 hover:border-pink-400 rounded-2xl p-4 flex gap-4 items-center shadow-md transition-all group relative">
                <Link 
                  href={`/massage/${region}/${encodeURIComponent(districtName)}/shop/${lShop.slug}${dongName ? `?dong=${encodeURIComponent(dongName)}` : ""}`} 
        ...
        <div className="text-center pt-2">
          <Link href={`/massage/${region}`} className="text-xs text-gray-500 hover:text-pink-600 transition-colors font-semibold">
            ← {regionName} 지역 메인 페이지로 돌아가기
          </Link>
        </div>
      </main>
    </div>
